...
class CGRUProperties(bpy.types.PropertyGroup):
    ...
    capacity = IntProperty(
        name='Capacity',
        description='Job Capacity',
        min=0,
        max=10000,
        default=1000)
    properties_needed = StringProperty(
        name='Properties Needed',
        description='Host Properties Needed to run the job',
        default='')
    ...
    maxruntasks = IntProperty(
        ...
        max=9999,
        default=-1)
    maxruntasksperhost = IntProperty(
        name='Max Tasks per Host',
        description='Maximum number of running tasks per Host',
        min=-1,
        max=9999,
        default=1)
    ...
    mov_name = StringProperty(
        ...
        default='',
        subtype="FILE_PATH")
    mov_width = IntProperty(name='Movie width', default=1920)
    mov_height = IntProperty(name='Movie height', default=1080)
    mov_codecs = EnumProperty(
        name="Movie codec",
        items=utils.get_movie_codecs)
    mov_audio = BoolProperty(name='Include Audio', default=True)